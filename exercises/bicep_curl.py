...
try:
    from utils.pose_utils import calculate_angle_3d, check_form_torso_relative  # type: ignore
except ImportError:
...
try:
    from training.train_advanced_model import AdvancedExerciseModel
except ImportError:
...
class BicepCurlExercise:
    def __init__(self, model_path_or_config, strict_thresh: float = 35):
        """
        Accept either:
          - str / Path  — model file path (legacy direct usage)
          - ExerciseConfig — passed by api_backend.py / main.py setup
        """
        # ── Resolve config and model path ────────────────────────────────
        if hasattr(model_path_or_config, 'model_path'):
            # ExerciseConfig object received
            self.config = model_path_or_config
            model_path   = model_path_or_config.model_path
            strict_thresh = model_path_or_config.strict_mode_threshold
        else:
            # Plain string path — create a minimal config stub so that
            # api_backend.py's exercise.config.name / .age_group don't crash.
            from exercises.base_exercise import ExerciseConfig
            model_path = str(model_path_or_config)
            self.config = ExerciseConfig(
                name='bicep_curl', age_group='adult',
                min_angle=50, max_angle=170,
                min_rep_time=1.0, max_rep_time=4.0,
                strict_mode_threshold=int(strict_thresh),
        ...
        # ── Model loading (advanced → basic → None) ──────────────────────
        self.model = None
        try:
            from training.train_advanced_model import AdvancedExerciseModel
    ...
    def calculate_rep_quality(self, elbow_angle: float, torso_angle: float, rep_time: Optional[float] = None) -> float:
        ...
        if elbow_angle < min_a:
            rom_score = (elbow_angle / min_a) * 100
        elif elbow_angle > max_a:
            rom_score = 100 - ((elbow_angle - max_a) / (200 - max_a)) * 100
        else:
            rom_score = 100
        score += rom_score * 0.4
    ...
    def process(self, shoulder: List[float], elbow: List[float], wrist: List[float], hip: Optional[List[float]] = None) -> Dict:
        ...
        try:
            ...
            self.logger.warning(f"Primary angle calculation failed: {e}")
            raw_angle = 0.0
        ...
        # 2. Form Analysis — secondary: hip→shoulder→elbow deviation
        torso_angle = 0.0
        if hip is not None:
            torso_result = check_form_torso_relative(shoulder, elbow, hip)
            torso_angle = float(torso_result.angle)

        # Tertiary angle: hip→shoulder→wrist (full arm swing from body line)
        tertiary_angle = 0.0
        if hip is not None:
            try:
                t_result = calculate_angle_3d(hip, shoulder, wrist)
                tertiary_angle = float(t_result.angle) if t_result.is_valid else 0.0
            except Exception as e:
                self.logger.warning(f"Tertiary angle calculation failed: {e}")

        # 3. AI Prediction — pass [primary, secondary, tertiary] to model
        ...
        if is_valid_angle and self.model:
            try:
                raw_model = self.model.get('model', self.model) if isinstance(self.model, dict) else self.model
                n_expected = getattr(raw_model, 'n_features_in_', 3)
                if n_expected == 3:
                    features = [[smoothed_angle, torso_angle, tertiary_angle]]
                elif n_expected == 2:
                    features = [[smoothed_angle, torso_angle]]
                else:
                    features = [[smoothed_angle]]

                prediction = raw_model.predict(features)[0]
                current_state = "UP" if prediction == 1 else "DOWN"

                if hasattr(raw_model, 'predict_proba'):
                    confidence = raw_model.predict_proba(features)[0][prediction] * 100
            except Exception as e:
                self.logger.warning(f"Model prediction failed, using geometric fallback: {e}")
    ...
    def process_frame(self, landmarks):
        """
        Adapter called by api_backend.py and main.py exercise-class path.

        Extracts the right-side joints from the full MediaPipe landmark list,
        delegates to self.process(), then re-packages the result as an
        exercises.base_exercise.ExerciseResult so the API response builder
        can access .counter, .primary_angle, .model_missing, etc.
        """
        from exercises.base_exercise import ExerciseResult as BaseResult

        # Right-side joints (MediaPipe indices)
        shoulder = landmarks[12]
        elbow    = landmarks[14]
        wrist    = landmarks[16]
        hip      = landmarks[24]

        result_dict = self.process(shoulder, elbow, wrist, hip)

        age_group     = getattr(self.config, 'age_group', 'adult')
        exercise_type = getattr(self.config, 'name', 'bicep_curl')

        return BaseResult(
            counter=result_dict['counter'],
            feedback=result_dict['feedback'],
            color=result_dict['color'],
            primary_angle=result_dict['elbow_angle'],
            secondary_angle=result_dict['torso_angle'],
            stage=result_dict['stage'],
            confidence=result_dict['confidence'],
            is_valid_rep=result_dict['is_valid_rep'],
            rep_quality=result_dict['rep_quality'],
